...
public class IntTree {
    ...
    // Method to count the number of left children in the tree
    ...
    private int countLeftNodes(IntTreeNode root) {
        if (root == null) {
            return 0;  // Base case: an empty subtree has no left nodes
        }

        int count = 0;
        // Check if there is a left child and increment the count if it exists
        if (root.left != null) {
            count += 1;  // This is a left child
        }

        // Recur on the left and right subtrees and add the results
        count += countLeftNodes(root.left);
        count += countLeftNodes(root.right);

        return count;
    }
    // Method to count even branch nodes
    public int countEvenBranches() {
        return countEvenBranches(overallRoot);
    }

    // Helper method to recursively count even branch nodes
    private int countEvenBranches(IntTreeNode node) {
        ...
        // Check if the current node is a branch node (has at least one child)
        boolean isBranch = (node.left != null || node.right != null);

        // If it's a branch and the value is even, count it
        int count = 0;
        if (isBranch && node.data % 2 == 0) {
            count = 1;
        }

        // Recur on the left and right children, summing the counts
        return count + countEvenBranches(node.left) + countEvenBranches(node.right);
    }

    // Method to double all positive values in the tree
    public void doublePositives() {
        doublePositives(overallRoot);
    }

    // Helper method to recursively double positive values
    private void doublePositives(IntTreeNode node) {
        ...
        // If the node's value is greater than 0, double it
        if (node.data > 0) {
            node.data *= 2;
        }
